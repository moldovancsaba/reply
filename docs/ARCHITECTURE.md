...
*   **Hub process:** `chat/server.js` serves the UI and API; it starts the **background worker** (`background-worker.js`) as a managed child. Recommended production-style run: LaunchAgent **`com.reply.hub`** via `make run` from the repo root (see [LOCAL_MACHINE_DEPLOYMENT.md](LOCAL_MACHINE_DEPLOYMENT.md) for plist paths, `tools/scripts/reply_service.sh`, logs under `~/Library/Logs/Reply/`, and the `/tmp/reply-hub.log` symlink). On **SIGTERM/SIGINT**, the hub closes the HTTP server and **`shutdownAllAsync`** stops managed children (worker, sidecars) before exit.
...
*   **Local LLM:** Ollama on `127.0.0.1:11434`; default model tag **`gemma4:e2b`** via `chat/ollama-model.js` and `REPLY_OLLAMA_MODEL`. Refine (`gemini-client.js`) uses the same resolver (local Ollama, not a cloud Gemini call in the current code path).
